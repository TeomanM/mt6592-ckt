...
def split_by_whitespace(line):    
    ...
    #
    line_split = [_f for _f in line.replace("\t"," ").replace("\n"," ").replace("\r"," ").split(" ") if _f]
    return line_split
...
def find_source_by_target(fd_log,out_log_path,file_path,type_label,semtk_domain_new_file):        
    ...
    cnt_cmd_c  = "| wc -l"
    is_exec_c = subprocess.getoutput(grep_cmd_c)
    cnt_exec_c = subprocess.getoutput("".join(["echo \"", is_exec_c," \"", cnt_cmd_c]))
...
def gen_type(fd_type,file_type,file_type_set):                                  
    ...
    is_seandroid_file_type = subprocess.getoutput("".join([grep_cmd,alps_path,"/",seandroid_root_policy_dir,"/*.te"]))
    is_semtk_product_file_type = subprocess.getoutput("".join([grep_cmd,alps_path,"/",semtk_root_policy_dir,"/*.te"]))
